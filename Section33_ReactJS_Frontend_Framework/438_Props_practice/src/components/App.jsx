...
function createCard(contact) {
  return (
    <Card 
      ...
      // 사용한다고해서 에러가 발생되지는 않지만 재사용될 수 없다.
      key={contact.id}
      // 따라서 id 값을 이용하기 위해 다른 속성 이름이 필요하다.
      id={contact.id}
      name={contact.name} 
      img={contact.imgURL} 
      tel={contact.phone} 
...
function App() {
  ...
  return (
    <div>
      ...
      {/* { contacts.map(function(contact) {return createCard(contact);}) } */}
      ...
      {/* 콜백 함수와의 관계를 설명하기 위해 위와 같이 설명하였고 어쨌든 결론적으로 아래와 같이 심플하게 표현할 수 있다. */}
